...
const Laptop = () => {
	...
	const { filteredLaptopData, LaptopData } = useSelector((state) => state.commerce);

	const truncate = (str) => {
		return str.length > 10 ? `${str.substring(0, 40)}...` : str;
	};

	const laptopData = filteredLaptopData.length > 0 ? filteredLaptopData : LaptopData;
				...
				{laptopData.map((x) => (
					<div className='product__top__container'>
						<Link className='product__inner__container' to={`${x.id}`}>
							<img className='product__image' src={require(`../assets/LaptopImage/${x.image}`)} alt='' />
							...
							<div className='product__info'>
								...
								<div className='price'>
									<p>Basket Price</p>
									<CurrencyFormat value={x.price} displayType={"text"} thousandSeparator={true} prefix={"$"} />
								</div>
							</div>
						</Link>
						<div className='heart'>
